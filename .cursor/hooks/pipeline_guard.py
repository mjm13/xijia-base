...
BODY_TIER_RE = re.compile(
    r"^\|\s*Tier\s*\|\s*\*?\*?(green-trivial|green|yellow|red)\*?\*?\s*\|",
...
# Shared helpers
# --------------------------------------------------------------------------- #
def _parse_frontmatter_block(text: str) -> dict[str, str] | None:
...
def _parse_frontmatter_type(text: str) -> str | None:
    ...
    if not block:
        return None
    value = block.get("type", "").lower()
    return value or None


def _parse_frontmatter_tier(text: str) -> str | None:
    block = _parse_frontmatter_block(text)
    if not block:
        return None
    value = block.get("tier", "").lower()
    return value if value in VALID_TIERS else None


def _parse_tier_from_body_table(text: str) -> str | None:
    match = BODY_TIER_RE.search(text)
    if not match:
        return None
...
def _resolve_tier(text: str, cli_tier: str) -> tuple[str | None, list[str]]:
    """Resolve tier: frontmatter is truth source; CLI may override only when FM absent."""
    issues: list[str] = []
    fm_tier = _parse_frontmatter_tier(text)
    body_tier = _parse_tier_from_body_table(text)

    if fm_tier and body_tier and fm_tier != body_tier:
        issues.append(f"frontmatter tier={fm_tier} 与正文 Tier={body_tier} 不一致")

    if fm_tier:
        if cli_tier and cli_tier != fm_tier:
            issues.append(f"CLI --tier={cli_tier} 与 frontmatter tier={fm_tier} 不一致")
        return fm_tier, issues

    if body_tier:
        issues.append("frontmatter 缺少 tier，请写入 YAML 头信息（正文 Tier 仅作展示）")
        if cli_tier and cli_tier != body_tier:
            issues.append(f"CLI --tier={cli_tier} 与正文 Tier={body_tier} 不一致")
        return body_tier, issues

    if cli_tier in VALID_TIERS:
        issues.append("frontmatter 缺少 tier，当前回退使用 CLI --tier")
        return cli_tier, issues

    return None, issues + ["未找到 tier（frontmatter / 正文 / CLI 均缺失）"]


def _is_red_requirement_text(text: str) -> bool:
    tier = _parse_frontmatter_tier(text)
    if tier == "red":
        return True
    if tier in {"green", "green-trivial", "yellow"}:
        return False
    # Legacy fallback before frontmatter tier was introduced
    fm_type = _parse_frontmatter_type(text)
    return fm_type in RED_TYPES or bool(RED_TEXT_RE.search(text))


def _red_requirements() -> list[str]:
    """Return inbox requirement names that are red-tier (need OpenSpec)."""
    if not INBOX_DIR.is_dir():
        return []
    found: list[str] = []
    for md in sorted(INBOX_DIR.glob("*.md")):
        text = md.read_text(encoding="utf-8", errors="ignore")
        if _is_red_requirement_text(text):
...
def _run_check_intake(req_path: str, tier: str) -> int:
    ...
    resolved_tier, tier_issues = _resolve_tier(text, tier.lower())
    for issue in tier_issues:
        print(f"[pipeline-guard] tier 警告：{issue}")
    if any("不一致" in issue for issue in tier_issues):
        print("  → 以 YAML frontmatter `tier` 为准，并同步正文「分级与判型」表格。")
        return 1
    if resolved_tier is None:
        print("[pipeline-guard] 未找到 tier，Gate-0 后须在 frontmatter 写入 tier。")
        return 1

    if resolved_tier == "green-trivial" and _GREEN_TRIVIAL_MARKER in text:
...
def main() -> int:
    ...
    if args.check_intake:
        if not args.req:
            ...
            return 2
        return _run_check_intake(args.req, args.tier.lower() if args.tier else "")
    if args.check_apply:
        if not args.change:
            print("[pipeline-guard] --check-apply 需要 --change <name>")
            return 2
        apply_tier = args.tier.lower() if args.tier else "red"
        return _run_check_apply(args.change, apply_tier)
