...
3. 若 `tier=red`：读取归档 change 的 design/tasks/specs。
4. 若 `tier=red` 且 change type 为 business/hybrid：将 `docs/openspec/changes/<name>/domain/*` 提升到 `docs/domain/*`。
5. 若 `tier=red`：运行 DDD 契约校验：
   - `python .cursor/skills/ddd-modeling/scripts/validate_domain_contracts.py --path "docs/domain"`
6. 执行动态合并协议（先 diff 再写）：
...
10. green/yellow 路由补充：
   - 命中 C 类（仅字段语义）时，允许只更新 `docs/domain/data-dictionary.md`，同样遵循动态合并协议 + 人工确认。
11. ADR 强制触点：若命中任一 Approval Gate 类权衡（新增关键外部依赖、权限/安全策略变更、跨上下文架构权衡），必须新增 `docs/decisions/*.md` 后才能宣告收尾完成（由 `pipeline_guard.py --check-release --req <requirement-file>` 机器兜底）。
12. 按需更新 `docs/capability-map.md` 与 `AGENTS.md`（命中其一即更新）：
...
13. 执行轻量代码↔文档漂移检查（delta spec vs domain/capability-map）；若 guard 给出 drift 提醒，需在验收记录写处置结论。
14. 运行 docs 卫生评分：
   - `python .cursor/skills/xijia-docs-score/scripts/score_docs.py`
15. 迁移需求状态到 shipped 并修复引用路径（仅在 Gate-2 已签字后执行）。
16. 提醒：commit 由用户触发，未 commit 不得进入下一需求。
