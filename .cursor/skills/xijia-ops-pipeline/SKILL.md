...
1. 分级与判型（写入 YAML frontmatter 的 `tier`/`type`；`tier=red` 时填 `openspec_change`；正文「分级与判型」章节须与 frontmatter 一致）
2. 出方案（Plan Mode）；写入 requirement 文档「实现方案」章节，不另建 `docs/plans/`
3. 触发 Gate-1，等待用户批准
4. 执行经验检索（实现前必做）：检索 `docs/pitfalls/*` 与 `docs/patterns/*`，将命中项写入 requirement「实现记录」；未命中写 `Experience Reuse: none`
5. 按触达栈建立测试约束（`backend-test` / `frontend-test`）并保持 TDD（`test-driven-development`）
6. 执行实现：新增/修改核心业务代码时，同步执行 comment-sync（见 E.0），不得留到 verify
7. verify（执行 E 子流程：verification/comment-sync guard/code-review/quality-judge）+ 文档同步 + 人工验收说明
   - 若命中 DDD C 类（仅字段语义），在 `sync-knowledge` 阶段更新 `docs/domain/data-dictionary.md`（动态合并，不是追加流水账）
8. 触发 Gate-2（验收签字后再迁移状态/归档）
9. sync-knowledge（`sync-knowledge`，`tier=green|green-trivial|yellow`）
10. 提醒用户触发 commit
...
5. apply 前经验检索（必做）：检索 `docs/pitfalls/*` 与 `docs/patterns/*`，将命中项写入 requirement「实现记录」；未命中写 `Experience Reuse: none`
6. apply（`openspec-superpowers-apply`）：新增/修改核心业务代码时，同步执行 comment-sync（见 E.0），不得留到 verify
7. verify（执行 E 子流程；🔴 默认强制 `requesting-code-review` + `quality-judge`）
8. sync（`openspec-sync-specs`）
9. 触发 Gate-2（验收签字后方可 archive）
10. archive（`openspec-archive-change`）
11. sync-knowledge（`sync-knowledge`，`tier=red`）
...
4. 若本轮实现存在踩坑或可复用方案，先在 requirement「实现记录」登记，收尾由 `sync-knowledge` 提炼为 `docs/pitfalls/*` 或 `docs/patterns/*`。
...
1. 读取当前需求文档（`docs/requirements/inbox/*.md`）确认 **frontmatter** 的 `tier` 与 `type`（正文表格须一致）。
