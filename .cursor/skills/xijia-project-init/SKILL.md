...
8. 浜у嚭鍒濆鍖栭攣鏂囦欢锛坄skills-lock.json`锛夋椂瑕佸湪鎶ュ憡涓鏄庣敤閫?
## 鎵ц姝ラ锛圫OP锛?
### 1) 浠撳簱淇濇姢妫€鏌ワ紙Guard锛?
...
### 5) 鎶€鏈爤纭鍚庡畨瑁呮妧鑳斤紙Skills Bootstrap锛?
娴佺▼锛?
1. 鍒楀嚭鍊欓€夋妧鑳芥潵婧愶紙`skills.sh`/GitHub锛?2. 瀵规瘡涓€欓€夎绠楄瘎鍒嗭紙0-100锛夛細
   - 鐢熸€侀€傞厤锛圕ursor 鍙敤锛? 30
   - 娲昏穬搴︼紙鏈€杩?90 澶╂湁鏇存柊锛? 25
   - 绀惧尯閲囩敤锛坰tars/download锛? 20
   - 瑙勮寖瀹屾暣搴︼紙frontmatter + references锛? 15
   - 瀹夎绋冲畾鎬э紙鍛戒护鍙噸鏀俱€佺洰褰曞彲棰勬祴锛? 10
3. 姣忎釜鎶€鏈爤閫夋嫨 Top 2-3锛堝幓閲嶏級
4. 瀹夎鍒?`.cursor/skills/`锛堝繀瑕佹椂鎼繍 `.agents/skills/`锛?5. 鏍￠獙 `SKILL.md` + frontmatter 涓€鑷存€?6. 鐢熸垚瀹夎鎶ュ憡锛堝€欓€夈€佽瘎鍒嗐€佸叆閫夈€佸け璐ラ噸璇曞懡浠わ級
...
璇存槑锛氬熀绾垮紩鎿庝繚鎸佹妧鏈爤鏃犲叧锛涘叿浣撴祴璇曞懡浠ゃ€佸伐绋嬬洰褰曚笌瀹炵幇缁嗚妭鐢卞凡瀹夎鐨勬妧鏈爤鎶€鑳芥壙杞姐€?
...
- In `auto install` mode:
  - if installed skills success count is 0, status must be `blocked` and cannot be `done`;
  - report install evidence (`commands + key outputs + retries/failure reasons`) in Init Status.
