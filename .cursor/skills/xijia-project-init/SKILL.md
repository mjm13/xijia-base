...
- `docs/memory/decisions.jsonl`（空 JSONL，init scaffold 创建）
...
- `docs/memory/decisions.jsonl` 由空模板生成，不得预填示例行
- `docs/memory/decisions.archive.jsonl` 不在 init 预生成，由 `memory_prune.py` 首次运行按需创建
...
1. `requiredFiles`：最小交付集关键文件逐项存在性检查
   ...
   - `docs/memory/decisions.jsonl` 必须存在（可为空文件）
